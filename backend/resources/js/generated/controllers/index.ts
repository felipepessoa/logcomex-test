...
type ActionWithMethods<
    P extends RouteParams | undefined,
    M extends string,
> = ActionFunction<P> & { [K in M]: ActionFunction<P> };
...
export function createActionWithMethods<
    P extends RouteParams | undefined = undefined,
    M extends string = string,
>(routes: MethodRoute[]): ActionWithMethods<P, M> {
    ...
    const createFn = (route: MethodRoute): ActionFunction<P> => {
        return ((...args: [P?, RouteOptions?]) => {
            ...
            const options = params
                ? (args[1] as RouteOptions)
                : (args[0] as RouteOptions);
